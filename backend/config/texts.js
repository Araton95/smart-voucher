module.exports = {
  texts: {
    // Webshop address URL
    missing_webshop: 'Webshop address is missing',
    invalid_webshop: 'Invalid webshop address',
    webshop_exists: 'Webshop address already exists',
    webshop_not_exists: 'Webshop with provided wallet not found',
    blocked_webshop: 'Webshop is blocked',

    // Partner
    missing_partners: 'Partner addresses are missing',
    invalid_partner: 'Invalid partner address',
    partner_not_exists: 'Webshop with provided partner wallet not found',
    blocked_partner: 'Partner is blocked',

    // Website URL
    missing_website: 'Website URL is missing',
    invalid_website: 'Invalid website URL',
    website_exists: 'Website URL already exists',

    // Email
    missing_email: 'Email is missing',
    invalid_email: 'Invalid email',
    email_exists: 'Email already exists',

    // Nonce
    missing_nonce: 'Nonce is missing',

    // Voucher id
    missing_voucher_id: 'Voucher id is missing',
    voucher_not_exists: 'Voucher with provided id not found',
    blocked_voucher: 'Voucher is blocked',

    // Voucher code
    missing_voucher_code: 'Voucher code is missing',
    not_allowed_redeem: 'Voucher not allowed to redeem',

    // Amount
    missing_amount: 'Amount is missing',

    // Signature
    missing_signature: 'Signature is missing',
    invalid_signature: 'Invalid signed data',
  }
}